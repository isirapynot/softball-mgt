...
function fmt(n: number | string) { return Number(n); }
...
export default async function SeasonStatsPage({ searchParams }: Props) {
  const { season } = await searchParams;

  // Parse comma-separated selected seasons, e.g. "Spring 2025,Summer 2025"
  const activeSeasons: string[] = season
    ? season.split(',').map(s => decodeURIComponent(s.trim())).filter(Boolean)
    : [];
  ...
  try {
    // All distinct seasons for the toggle pills
    const [seasonRows] = await pool.query<never[]>(
      `SELECT DISTINCT season FROM games WHERE season IS NOT NULL AND season != '' ORDER BY season DESC`
    );
    allSeasons = (seasonRows as { season: string }[]).map(r => r.season);

    if (activeSeasons.length > 0) {
      // Filter to the selected seasons using IN (...)
      const placeholders = activeSeasons.map(() => '?').join(', ');
      const [data] = await pool.query<never[]>(
        `SELECT
           p.id as player_id, p.name as player_name,
           COUNT(DISTINCT bs.game_id) as games,
           COALESCE(SUM(bs.ab), 0) as ab, COALESCE(SUM(bs.h), 0) as h,
           COALESCE(SUM(bs.doubles), 0) as doubles, COALESCE(SUM(bs.triples), 0) as triples,
           COALESCE(SUM(bs.hr), 0) as hr, COALESCE(SUM(bs.r), 0) as r,
           COALESCE(SUM(bs.rbi), 0) as rbi, COALESCE(SUM(bs.bb), 0) as bb,
  ...
  const heading = activeSeasons.length === 0
    ? 'All-Time Statistics'
    : activeSeasons.length === 1
      ? `${activeSeasons[0]} Statistics`
      : `${activeSeasons.join(' + ')} Statistics`;

  const runsCls = 'bg-gray-100';
        ...
        <div>
          ...
          {activeSeasons.length > 1 && (
            <p className="text-sm text-gray-400 mt-0.5">
              Stats aggregated across {activeSeasons.length} selected seasons
            </p>
          )}
        </div>
        <Link href="/" className="text-sm text-gray-400 hover:text-gray-600 mt-1">← Dashboard</Link>
      </div>

      {/* Season toggle pills */}
      {allSeasons.length > 0 && (
        <div className="mb-5">
          <SeasonSelector seasons={allSeasons} />
        </div>
      )}
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-lg font-medium">
            {activeSeasons.length > 0
              ? `No stats recorded for the selected season${activeSeasons.length > 1 ? 's' : ''}.`
              : 'No stats recorded yet.'}
          </p>
          <p className="text-sm mt-1">Stats are entered by the admin after each game.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-400">
              <tr>
                ...
                <th className={thCls + ' ' + runsCls} title="Runs">R</th>
                <th className={thCls  + ' ' + runsCls} title="Runs Batted In">RBI</th>
              ...
              {rows.map((row, i) => {
                ...
                return (
                  <tr key={row.player_id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                    ...
                    <td className={tdCls + ' ' + runsCls}>{fmt(row.r)}</td>
                    <td className={tdCls + ' ' + runsCls}>{fmt(row.rbi)}</td>
